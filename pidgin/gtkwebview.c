...
//	g_signal_connect(G_OBJECT(webview), "key_press_event",
//	                 G_CALLBACK(gtk_foo_bar_key_press_event), NULL);
...
pidgin_webview_get_type(void)
{
	...
	if (G_UNLIKELY(mview_type == 0)) {
		...
		mview_type = g_type_register_static(gtk_text_view_get_type(),
...
pidgin_webview_load_html_string(PidginWebView *webview, const char *html)
{
	...
	stripHTMLTags(ht, strlen(ht));
	//fprintf(stderr, "pidgin_webview_load_html_string text: %s\n", ht);
buffer = gtk_text_view_get_buffer (GTK_TEXT_VIEW (webview));
gtk_text_buffer_get_end_iter(buffer, &iter);
gtk_text_buffer_insert(buffer, &iter, ht, -1);
	g_free(ht);	
	//fprintf(stderr, "pidgin_webview_load_html_string: %s\n", html);
	g_return_if_fail(webview != NULL);
...
pidgin_webview_append_html(PidginWebView *webview, const char *html)
{
	...
	GtkTextIter iter;
	char *ht;
	//fprintf(stderr, "pidgin_webview_append_html: %s\n", html);
	ht = strdup(html);
	
	stripHTMLTags(ht, strlen(ht));
	//fprintf(stderr, "pidgin_webview_append_html text: %s\n", ht);
buffer = gtk_text_view_get_buffer (GTK_TEXT_VIEW (webview));
gtk_text_buffer_get_end_iter(buffer, &iter);
gtk_text_buffer_insert(buffer, &iter, ht, -1);
	g_free(ht);
...
pidgin_webview_get_body_html(PidginWebView *webview)
{
	...
	GtkTextIter from, to;
buffer = gtk_text_view_get_buffer (GTK_TEXT_VIEW (webview));
gtk_text_buffer_get_start_iter(buffer, &from);
gtk_text_buffer_get_end_iter(buffer, &to);
stripped = gtk_text_buffer_get_text( buffer, &from, &to, FALSE);
/*	g_return_val_if_fail(webview != NULL, NULL);

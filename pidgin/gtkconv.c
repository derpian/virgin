...
pidgin_webview_clear_text(webview);
#if 0
	/* TODO WebKit */
...
*/
displaying = g_strdup_printf("%s\n", purple_message_get_contents(pmsg));
h = purple_message_get_time(pmsg);
h_tm = localtime(&h);
			h_s = purple_utf8_strftime(
				"%X", h_tm);
//purple_utf8_strftime("%Y-%m-%dT%H:%M:%SZ", h)
pidgin_webview_add_text(PIDGIN_WEBVIEW(gtkconv->webview), displaying, purple_message_get_author(pmsg), purple_message_get_author_alias(pmsg), h_s);
